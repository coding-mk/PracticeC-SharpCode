 ...
 */
namespace DifferentBitsSumPairwise
{
    public class Program
    {
        ...
        {
            int[] A = { 1, 3, 5 };
            int result = CountDifferentBitsSumPairwise(A);
            Console.WriteLine(result);
        }

        private static int CountDifferentBitsSumPairwise(int[] A)
        {
            ...
            for (int i = 0; i < 32; i++)
            {
                res += ((long)count1[i] * (A.Length - count1[i])) % 1000000007;
            }
